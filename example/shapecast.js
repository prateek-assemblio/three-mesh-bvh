...
function init() {
	...
	const knotGeometry2 = new THREE.TorusKnotGeometry( 3, 0.2, tubularSegments, radialSegments );
	const material = new THREE.MeshPhongMaterial( { color: 0xffffff, side: THREE.DoubleSide } );
	targetMesh = new THREE.Mesh( knotGeometry, material );
	targetMesh2 = new THREE.Mesh( knotGeometry2, material );
	targetMesh.geometry.computeBoundsTree();
	targetMesh2.geometry.computeBoundsTree();
	...
	// resource URL
		...
		function ( gltf ) {
			...
			customBoxGeometry = gltf.scene.children[ 0 ].geometry;
	...
	const torus = new THREE.Mesh( new THREE.TorusGeometry( 3, 1, 16, 100 ), shapeMaterial );
	scene.add( torus.geometry );

	// const customBox = new THREE.Mesh( customBoxGeometry, shapeMaterial );
	// scene.add( customBoxGeometry );
...
function render() {
	...
	const transformMatrix2 =
			new THREE.Matrix4()
				.copy( targetMesh2.matrixWorld ).invert()
				.multiply( shape.matrixWorld );

	const transformMatrixBox =
				new THREE.Matrix4()
					.copy( targetMesh2.matrixWorld ).invert()
					.multiply( shape.matrixWorld );
	if ( s === 'sphere' ) {
		...
		const hit = targetMesh.geometry.boundsTree.intersectsSphere( sphere, transformMatrix );
		const hit2 = targetMesh2.geometry.boundsTree.intersectsSphere( sphere, transformMatrix );

		shape.material.color.set( hit || hit2 ? 0xE91E63 : 0x666666 );
		...
		const hit2 = targetMesh2.geometry.boundsTree.intersectsBox( box, transformMatrix2 );
		// if ( hit ) {

		// 	console.log( shapes ); console.log( 'hit shape' );

		// }

		// if ( hit2 ) console.log( 'hit shape 2' );
		// console.log( hit2 ? 'hit box' : null );
		shape.material.color.set( hit || hit2 ? 0xE91E63 : 0x666666 );
		shape.material.emissive.set( 0xE91E63 ).multiplyScalar( hit ? 0.25 : 0 );

	} else if ( s === 'geometry' ) {

		console.log( 'is geometry' );
		const hit = targetMesh.geometry.boundsTree.intersectsGeometry( shape.geometry, transformMatrix );
		const hit2 = targetMesh2.geometry.boundsTree.intersectsGeometry( shape.geometry, transformMatrix2 );
		shape.material.color.set( hit || hit2 ? 0xE91E63 : 0x666666 );
		console.log( "contact" );
		shape.material.emissive.set( 0xE91E63 ).multiplyScalar( hit ? 0.25 : 0 );
